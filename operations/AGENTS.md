...
| `karpenter-eks/` | **Karpenter on Amazon EKS** — just-in-time node-lifecycle autoscaling: `NodePool` / `EC2NodeClass` / `NodeClaim` CRDs, the AWS install/IAM surface, the disruption engine (consolidation / drift / expiration / interruption), observability, and troubleshooting trees; ships 3 read-only triage scripts under `tools/` and a 5-agent team in `../.claude/agents/` (see `karpenter-eks/AGENTS.md`) |
