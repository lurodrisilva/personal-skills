...
[![Skills](https://img.shields.io/badge/skills-36-blue)](#available-skills)
...
|-------|----------|-------|------------------|
| [platform-architect](platform-engineering/platform-architect/SKILL.md) | Internal Developer Platform (any cloud) | Distinguished platform-engineering architecture — the capstone that decides *what* the platform offers and delegates the *how* to the sibling skills: platform-as-a-product strategy, IDP reference architecture (5 planes), golden paths, Team Topologies, DORA/SPACE + adoption metrics, ADR/RFC governance, CNCF maturity. Ships 3 read-only assessment scripts under `tools/` + 6 companion subagents in `.claude/agents/` | Platform-as-a-product; five IDP planes (Developer Control / Integration & Delivery / Monitoring & Logging / Security / Resource); golden paths / paved roads (guardrails-not-gates); Team Topologies (X-as-a-Service / collaboration / facilitating, cognitive load, Conway's law); DORA four keys + SPACE + platform NPS/adoption; technology radar (adopt/trial/assess/hold); Wardley mapping; ADR (MADR) + RFC; CNCF Platform Engineering Maturity Model (Investment/Adoption/Interfaces/Operations/Measurement × Provisional→Operational→Scalable→Optimizing); Backstage catalog; delegates to terraform-iac / crossplane / gitops-argocd / observability-stack / aws-finops / azure-finops / kubernetes-security |
...
| [github-cli](platform-engineering/github-cli/SKILL.md) | GitHub (`gh` CLI) | GitHub CLI usage for ad-hoc ops, Makefile glue, CI steps, PR/release automation — command-group grammar, token-precedence auth, `--json`/`--jq`/`--template` over table-scraping, the `gh api` escape hatch, repo-context determinism, extension supply-chain hygiene. Siblings `aws-cli`/`azure-cli`; boundary partner `github-actions`. Ships 3 read-only `gh` triage scripts under `tools/` + 6 companion subagents in `.claude/agents/` | GitHub CLI (`gh`), `gh auth login` (web/device/`--with-token`/`--hostname` GHES) + token precedence (`GH_TOKEN`>`GITHUB_TOKEN`>keyring; `GH_ENTERPRISE_TOKEN`), fine-grained PAT vs classic vs ephemeral `GITHUB_TOKEN`, repo-context (`-R`/`GH_REPO`/`set-default`), `--json`+built-in `--jq`+Go-`--template` (`tablerow`/`timeago`/`hyperlink`), `gh api` REST+GraphQL (`-f` string vs `-F` typed, GET→POST, `--paginate`/`--slurp`), `gh config`+`GH_*` env, aliases + `gh extension --pin` (unverified code), `gh pr`/`issue`/`release --verify-tag`, Actions (`workflow run`/`run watch --exit-status`/`secret` libsodium/`variable`/`cache`/`attestation`), gh-in-Actions `GH_TOKEN`+least-privilege `permissions:`, exit codes 0/1/2/4, GitHub MCP Server (`github/github-mcp-server`, remote + local, toolsets, `--read-only`) |
...
| [crossplane](platform-engineering/crossplane/SKILL.md) | Kubernetes (any distro) | **Building** a Crossplane control plane — Managed Resources + lifecycle, XRDs as versioned platform APIs, function-pipeline Compositions, packages, safe-start MR activation, provider development, GitOps delivery, and testing. **Crossplane v2-first** (namespaced XRs, Claims removed, functions-only composition) with v1 flagged. Ships 4 read-only `kubectl` triage scripts under `tools/` + a read-only MCP surface (`kubernetes-mcp-server --read-only`) and 6 companion subagents in `.claude/agents/`. Complements `addons-and-building-blocks` (which *consumes* managed resources) | Crossplane v2, `apiextensions.crossplane.io` (XRD `/v2`, Composition `/v1`), Composition Functions (`function-patch-and-transform` / `-auto-ready` / `-environment-configs`), `pkg.crossplane.io` packages (Provider / Configuration / Function), safe-start **`ManagedResourceDefinition` + `ManagedResourceActivationPolicy`** (`spec.activate[]`), `crossplane xpkg build/push`, ImageConfig + Cosign, `managementPolicies`, ProviderConfig + workload identity, **provider development** (`provider-template`, Upjet vs native, `ExternalClient` CRUD, `SetupGated`), `provider-kubernetes` `Object` + `InjectedIdentity`, observability-as-code, x-generation, `crossplane composition render` / `resource validate` / `resource trace`, ArgoCD delivery |
...
| [azure-cli](platform-engineering/azure-cli/SKILL.md) | Azure (CLI / `az`) | Azure CLI usage for ad-hoc ops, Makefile glue, CI pipelines, AKS bootstraps — command-group grammar, identity-first auth (managed identity / OIDC over long-lived secrets), client-side JMESPath `--query` + output formats, config + `AZURE_*` env surface, extensions, waiters, sovereign clouds. Sibling of `aws-cli`. Ships 3 read-only `az` triage scripts under `tools/` + 5 companion subagents in `.claude/agents/` | Azure CLI (`az`), `az login` (interactive/WAM/device-code, service-principal cert vs secret, `--identity` managed identity, `--federated-token` OIDC), `az ad sp create-for-rbac` least-privilege scoping, `az account` / `az cloud` (sovereign), client-side JMESPath `--query`, `-o json/tsv/table/none` (table/tsv gotchas), `az config` + `AZURE_{SECTION}_{NAME}` env, `az extension` + dynamic install, `--no-wait` + `az … wait` + `--ids @-`, `azure/login@v2` OIDC, `az rest` ARM escape hatch, Azure MCP Server (`microsoft/mcp`, reuses `az login`) |
...
| [aws-finops](platform-engineering/aws-finops/SKILL.md) | AWS (FinOps) | **FinOps on AWS** — cloud financial management that maximizes the *business value* of spend, not just cuts it. The **FinOps Framework** (Inform→Optimize→Operate; four domains), the **AWS Well-Architected Cost Optimization** pillar (5 principles), and **FOCUS** as the billing schema. Owns allocation, usage + rate optimization, budgeting/forecasting, governance, EKS + AI cost. Sibling of `azure-finops`; delegates CLI mechanics to `aws-cli`. Ships 3 read-only `aws` triage scripts under `tools/` and 5 companion subagents in `.claude/agents/` | **Cost & Usage Report (CUR 2.0)** + **Data Exports (FOCUS 1.0)** → S3/Athena/QuickSight, **Cloud Intelligence Dashboards** (CUDOS/CID/KPI), **Cost Explorer** (analysis / forecast / RI-SP + rightsizing recs / CE API), **Cost Optimization Hub** (consolidated rightsizing / idle / SP / RI at your rates), **Compute Optimizer** (CloudWatch-metric rightsizing; memory needs the CloudWatch agent), **Trusted Advisor** cost checks, **AWS Budgets** + budget actions, **Cost Anomaly Detection**, cost allocation tags + **Cost Categories** + **Organizations**, **Billing Conductor** (pro-forma chargeback), **Savings Plans vs RIs vs Spot** + **Graviton** (coverage 60–85% / utilization >90%), **EKS Split Cost Allocation Data** (`aws:eks:*` tags / `SplitLineItem/SplitUsage`), Bedrock/SageMaker/GPU cost-per-token; `BilledCost`/`EffectiveCost`/`PricingCategory`/`CommitmentDiscountCategory` FOCUS columns, **AWS FinOps Agent** (preview) |
| [opencost](platform-engineering/opencost/SKILL.md) | Kubernetes (any distro — EKS/AKS/GKE/OCI/on-prem) | **OpenCost** (CNCF) — the *measurement plane* for Kubernetes cost, and the reference implementation of the **OpenCost Specification**. Owns the tool itself: deploy it, price it correctly, query it, extend it, fix it. Spine is **name the pricing source**: with no cloud integration you are reading **public on-demand list price**, not your bill. Complements `operations/kubernetes-finops` (which *acts* on these numbers). Ships 3 read-only triage scripts under `tools/` + a read-only MCP surface and 5 companion subagents in `.claude/agents/` | Helm `opencost-charts/opencost` + raw manifest + `ghcr.io/opencost/opencost`/`-ui` + metrics-only `prometheus-community/prometheus-opencost-exporter`; ports **9003** (API + `/metrics`) / **9090** (UI) / **8081** (MCP); the `job_name: opencost` Prometheus scrape config + `extraScrapeConfigs.yaml`; emitted metrics (`node_cpu_hourly_cost` / `node_ram_hourly_cost` / `node_total_hourly_cost` / `container_cpu_allocation` / `container_memory_allocation_bytes` / `pod_pvc_allocation` / `pv_hourly_cost` / `kubecost_load_balancer_cost` / `kubecost_node_is_spot`) + consumed KSM/node-exporter series; multi-cluster single-source-of-data (Thanos/Cortex/Mimir + `CURRENT_CLUSTER_ID_FILTER_ENABLED`/`PROM_CLUSTER_ID_LABEL`/`CLUSTER_ID`); `cloud-integration.json` + the `cloud-costs` secret (AWS Athena/CUR, Azure storage+RateCard, GCP BigQuery, OCI) with `authorizerType` `AWSAccessKey`/`AzureAccessKey`/`GCPServiceAccountKey`/**`GCPWorkloadIdentity`** + IRSA; `opencost.customPricing` for on-prem; the API (`/allocation`, `/allocation/compute`, `/assets`, `/cloudCost`, `/customCost/*` with `window`/`aggregate`/`step`/`resolution`/`accumulate`/`includeIdle`/`shareIdle`/`idleByNode`/`filter`) + `listCost` vs `netCost` vs amortized variants; `kubectl cost` (krew); CSV (`EXPORT_CSV_FILE`) + Parquet CronJob export; carbon estimates in KG CO2e (Cloud Carbon Footprint); the plugin framework (Datadog / OpenAI / MongoDB Atlas); the **OpenCost MCP server** (`get_allocation_costs` / `get_asset_costs` / `get_cloud_costs`) |
| [terraform-iac](platform-engineering/terraform-iac/SKILL.md) | Terraform / OpenTofu | **Infrastructure as Code** through a **plan → review → gated apply** lifecycle — module authoring, state & backends, providers & auth, plan review + policy-as-code, testing, drift. Ships 3 read-only `terraform` triage scripts under `tools/` and 5 companion subagents in `.claude/agents/` | HCL2 modules (typed vars + `validation`, `for_each`/`dynamic`, `moved`, module registry + version constraints), remote state + locking (**S3+DynamoDB** / **azurerm** / **gcs** / **Terraform Cloud/HCP**), `import` blocks, `required_providers` + committed `.terraform.lock.hcl`, **OIDC / assume-role / workload-identity** auth (no static keys), `terraform plan -detailed-exitcode` review + **OPA/Conftest/Sentinel** policy-as-code, `prevent_destroy`/`-target` guard + PR-gated apply, native `terraform test` (`.tftest.hcl`) + **terratest** + **tflint** + **tfsec/checkov/trivy**, `plan -refresh-only` drift; MCP = HashiCorp `terraform-mcp-server` (registry docs, read-only) + GitHub MCP |
...
| [kubernetes-finops](operations/kubernetes-finops/SKILL.md) | Kubernetes (any distro — EKS/AKS/GKE/on-prem) | **FinOps on Kubernetes** — allocating and optimizing the cost of running containers on a shared cluster. Clusters run at ~20–30% utilization because **requests** are set far above usage; this skill owns the fix: the container cost model (**allocated / idle / shared** split on `max(request,usage)`), cost allocation, right-sizing, node efficiency, waste, governance. Cost lens counterpart to `kubernetes-operations`; hands node lifecycle to `karpenter-operations` and the cloud bill to `azure-finops`. Ships 3 read-only `kubectl` triage scripts under `tools/` and 5 companion subagents in `.claude/agents/` | **OpenCost** (CNCF) / **Kubecost** on Prometheus (Allocation by namespace/label/pod), FinOps Foundation container-cost model (allocated/idle/shared), **requests vs limits** + **QoS classes**, right-sizing via **VPA** (recommendation mode) / **Goldilocks** / **KRR** (p95/p99), **HPA** / **KEDA** scale-to-zero / bin-packing / descheduler, **Spot** node pools + Arm64, **ResourceQuota** / **LimitRange** + Kyverno/Gatekeeper require-requests/labels, showback→chargeback, FOCUS rollup, `kubectl top` waste signals |
| [karpenter-operations](operations/karpenter-operations/SKILL.md) | AWS EKS + Azure AKS | **Operating Karpenter** across its two first-class clouds — just-in-time node-lifecycle autoscaling that provisions right-sized VMs from pending-pod constraints instead of scaling fixed node groups. Shared core API with a per-cloud NodeClass; covers install/identity, the disruption engine, observability, and troubleshooting on both (AWS self-hosted + Azure **Node Auto Provisioning (NAP)** managed). Owns Karpenter; hands generic HPA/VPA/KEDA + Cluster Autoscaler to `kubernetes-operations`. Ships 3 read-only triage scripts under `tools/` and 5 companion subagents in `.claude/agents/` | `NodePool` / `NodeClaim` (`karpenter.sh/v1`) + per-cloud NodeClass — `EC2NodeClass` (`karpenter.k8s.aws/v1`) / `AKSNodeClass` (`karpenter.azure.com/v1beta1`); requirements (`minValues`, `karpenter.sh/capacity-type`, `karpenter.k8s.aws/instance-*` / `karpenter.azure.com/sku-*`); **AWS** — AMI alias pinning + tag discovery (`karpenter.sh/discovery`) + IMDSv2 + SQS interruption + Pod Identity/IRSA + CloudFormation; **Azure** — NAP (`az aks --node-provisioning-mode Auto`, CNI-Overlay+Cilium, Workload Identity, `imageFamily`); disruption (consolidation `WhenEmptyOrUnderutilized` / drift / expiration / interruption / budgets / `do-not-disrupt` / `terminationGracePeriod`); `karpenter_*` metrics + AKS control-plane `karpenter-events`; Cluster-Autoscaler + self-hosted→NAP migration |
| [gitops-argocd](operations/gitops-argocd/SKILL.md) | Kubernetes (Argo CD + Flux) | **GitOps continuous delivery** — Git as the single source of truth, cluster continuously converges; Applications, the sync engine, health/drift, multi-cluster fan-out, progressive delivery + promotion, and the Flux sibling. Ships 3 read-only `kubectl`/`argocd` triage scripts under `tools/` and 5 companion subagents in `.claude/agents/` | **Argo CD** `Application` (`argoproj.io/v1alpha1`, multi-source Helm/Kustomize/directory, **app-of-apps**), **AppProject** tenancy/blast-radius, sync policy (`prune`/`selfHeal`) + **sync waves** + PreSync/Sync/PostSync/SyncFail **hooks** + `ServerSideApply`, custom-Lua **health** + `ignoreDifferences` + OutOfSync/Degraded triage, **ApplicationSet** generators (list/cluster/git/matrix/merge/scmProvider/pullRequest) + RBAC/SSO tenancy, **Argo Rollouts** canary/blue-green + **AnalysisTemplate** + **Image Updater** + sync windows, **Flux** (`GitRepository`/`Kustomization`/`HelmRelease` + **Flagger**); MCP = `mcp-for-argocd` (`MCP_READ_ONLY`) + `kubernetes-mcp-server --read-only` + GitHub MCP |
| [observability-stack](operations/observability-stack/SKILL.md) | Kubernetes / any (OSS) | **Vendor-neutral OSS observability** (Prometheus/Grafana/OTel/Loki/Tempo/Alertmanager — the **LGTM** stack) + **SLOs** — three signals correlated into one context, alert on symptoms + SLO burn rate. The OSS counterpart to `dynatrace`. Ships 3 read-only config-validator scripts under `tools/` and 5 companion subagents in `.claude/agents/` | **PromQL** recording + alerting rules, Prometheus Operator **`ServiceMonitor`/`PodMonitor`/`PrometheusRule`**, relabeling + cardinality control + remote-write/**Thanos**/**Mimir**, **OpenTelemetry Collector** (receivers/processors/exporters/connectors, OTLP, `memory_limiter`+`batch`+`k8sattributes`+**tail-sampling**, semantic conventions), **Loki**/LogQL + **Tempo**/TraceQL + **exemplars** trace↔log correlation, **Grafana dashboards-as-code** (JSON/provisioning/grafana-operator/Terraform) + **RED/USE**, **SLI/SLO/error-budget** with **Sloth**/**OpenSLO** + multi-window **burn-rate** alerts, **Alertmanager** routing/inhibition/silences; MCP (read-only, community) = Prometheus MCP + `grafana/mcp-grafana` + `kubernetes-mcp-server --read-only` |
