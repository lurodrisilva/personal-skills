...
| File | Description |
|------|-------------|
| `validate-skills.sh` | Bash validator that enforces the SKILL.md contract on every file under each `DOMAIN_DIRS` tree (`coding/`, `platform-engineering/`, `operations/`, `security/`, `networking/`). Exit code = error count; `0` = all checks passed |
...
- The `DOMAIN_DIRS=(coding platform-engineering operations security networking)` array drives which trees are validated; the orphan-directory check and the per-`SKILL.md` validation loop both iterate it. Add a new top-level domain to that array to extend coverage (and a missing domain dir is itself an error, so create the directory with a skill in the same change). A subdirectory with no `SKILL.md` but one or more `*-expertise.md` / `*-workflow.md` notes is accepted as a knowledge dir via the orphan-directory exception.
...
### Internal
- `../coding/**/SKILL.md`, `../platform-engineering/**/SKILL.md`, `../operations/**/SKILL.md`, `../security/**/SKILL.md`, `../networking/**/SKILL.md` — every file walked by the validator (one glob per `DOMAIN_DIRS` entry).
