...
- **`scripts/validate-skills.sh` does NOT walk this directory** (its `DOMAIN_DIRS` covers `coding/`, `platform-engineering/`, `operations/`, and `security/`, not `ai/`). After editing, manually verify:
...
- `../../scripts/validate-skills.sh` — does not validate this file; its `DOMAIN_DIRS` covers `coding/`, `platform-engineering/`, `operations/`, and `security/`, not `ai/`. Validate manually (or add `ai` to `DOMAIN_DIRS` to opt in).
