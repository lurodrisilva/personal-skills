...
## Purpose
AI-tooling skills — guidance for tools that AI coding assistants drive or integrate with (knowledge-graph builders, MCP servers, agent-facing CLIs). Each subdirectory contains exactly one `SKILL.md` that Claude Code / opencode auto-loads when a project matches the skill's `description`. This is a domain directory parallel to `coding/` and `platform-engineering/`.
...
- **`scripts/validate-skills.sh` does NOT walk this directory** — it only walks `coding/` (same coverage gap as `platform-engineering/`). After editing any SKILL.md here, manually verify:
